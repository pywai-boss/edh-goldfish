...
const commanderDomain =
  typeof require !== "undefined"
    ? require("./src/domain/commander.js")
...
const {
  ...
  getCommanderCandidatePool: domainGetCommanderCandidatePool,
  ...
  getOracleTextLower: domainGetOracleTextLower,
  getPartnerWithTargetKey: domainGetPartnerWithTargetKey,
  getTypeLineText: domainGetTypeLineText,
  isBackgroundCommanderCard: domainIsBackgroundCommanderCard,
  isCommandZoneCard: domainIsCommandZoneCard,
...
const {
  ...
  simulateCommanderCastAccess: domainSimulateCommanderCastAccess,
  simulateCurveAccess: domainSimulateCurveAccess,
  summarizeHand: domainSummarizeHand,
} = simulationDomain;

const simulationRenderDomain =
  typeof require !== "undefined"
...
const scryfallData =
  typeof require !== "undefined"
    ? require("./src/data/scryfall.js")
    : window.EDHCardDomain.scryfall;
...
function detectCommandersFromSections(parsedDeck) {
  return domainDetectCommandersFromSections(parsedDeck);
}

function getCommanderKey(commander) {
  return domainGetCommanderKey(commander);
}

function getCommanderKeySet(commanders = []) {
  return domainGetCommanderKeySet(commanders);
}

function isOutsideDeckCard(card) {
  return domainIsOutsideDeckCard(card);
}

function isCommandZoneCard(card, commanderKeySet = new Set()) {
  return domainIsCommandZoneCard(card, commanderKeySet);
}

function getCommanderColorIdentity(commanders) {
  return domainGetCommanderColorIdentity(commanders);
}
...
function simulateCommanderCastAccess(
  ...
  return domainSimulateCommanderCastAccess(deck, commanders, iterations, commanderColorIdentity);
}
...
function getSimulationRenderer() {
  if (!simulationRenderer) {
    simulationRenderer = simulationRenderDomain.createSimulationRenderer({
      summarizeDeck,
...
function renderDeckMetrics(parsed, result) {
  return getSimulationRenderer().renderDeckMetrics(parsed, result);
}
...
function renderLandChart(result) {
  return getSimulationRenderer().renderLandChart(result);
}

function renderManaStats(result) {
  return getSimulationRenderer().renderManaStats(result);
}

function createSimulationFeatures(args = {}) {
  return getSimulationRenderer().createSimulationFeatures(args);
}
...
function renderColorCurveAnalysis(
  ...
  return getSimulationRenderer().renderColorCurveAnalysis(
    colorResult,
    curveResult,
    commanderCastResult,
    simulationResult,
...
function renderSampleHands(result) {
  return getSimulationRenderer().renderSampleHands(result);
}
...
function isLikelyCommanderCandidate(card) {
  return domainIsLikelyCommanderCandidate(card);
}

function getDeckRequiredColorIdentityFromNonCommanders(parsed) {
  return domainGetDeckRequiredColorIdentityFromNonCommanders(parsed);
}

function getTypeLineText(card) {
  return domainGetTypeLineText(card);
}

function getOracleTextLower(card) {
  return domainGetOracleTextLower(card);
}

function isBackgroundCommanderCard(card) {
  return domainIsBackgroundCommanderCard(card);
}

function isDoctorCommanderCard(card) {
  return domainIsDoctorCommanderCard(card);
}

function getPartnerWithTargetKey(card) {
  return domainGetPartnerWithTargetKey(card);
}

function getCommanderPartnerProfile(card) {
  return domainGetCommanderPartnerProfile(card);
}

function canCardsFormLegalPartnerPair(cardA, cardB) {
  return domainCanCardsFormLegalPartnerPair(cardA, cardB);
}

function getCandidateColorIdentity(card) {
  return domainGetCandidateColorIdentity(card);
}

function getCombinedCommanderColorIdentity(cards) {
  return domainGetCombinedCommanderColorIdentity(cards);
}

function getCommanderCandidatePool(parsed) {
  return domainGetCommanderCandidatePool(parsed);
}

function getCommanderOptions(parsed) {
  return domainGetCommanderOptions(parsed);
}

function dedupeNames(names = []) {
  return domainDedupeNames(names);
}

function cardFitsCommanderIdentity(card, commanderColorSet) {
  return domainCardFitsCommanderIdentity(card, commanderColorSet);
}

function getCommanderLegalityIssueMessage(type, cards = [], details = '') {
  return domainGetCommanderLegalityIssueMessage(type, cards, details);
}

function validateCommanderDeckLegality(parsed, commanders = []) {
  return domainValidateCommanderDeckLegality(parsed, commanders, summarizeDeck);
}
